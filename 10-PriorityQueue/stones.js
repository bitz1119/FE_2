 ...
 class PriorityQueue{
    ...
    findHighestAmongParentChild(currIndex){
        ...
        if(this.rightChild(currIndex) <= this.pq.length-1){
            ...
            console.log("remove",max,parent,leftChild);
            if(max === parent){
                return currIndex;
            }
            else if(max === leftChild){
...
var lastStoneWeight = function(stones) {
    ...
    for(let i = 0;i<stones.length;i++){
        pq.addElement(stones[i]);
    }
    
    while(pq.pq.length > 1){
        // remove top elements
        let element1 = pq.pq[0];
        pq.removeElement();
        
        let element2 = pq.pq[0];
        pq.removeElement();

        console.log(element1,element2)
        console.log(pq.pq)
        if(element1 - element2 != 0){
            pq.addElement(element1 - element2)
        }
        
    }
    
    return pq.pq[0]!=undefined?pq.pq[0]:0;
